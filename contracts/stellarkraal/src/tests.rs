...
fn init(
    ...
    client.initialize(admin, oracle, token, treasury, &6000u32, &8000u32, &1u32, &0u64);
}
...
fn test_reinit_state_unchanged_after_failed_second_init() {
    ...
    let result = client.try_initialize(
        ...
        &0u64,    // twap_window_ledgers = 0 → default
    );
...
fn test_initialize_zero_admin_fails() {
    ...
    client.initialize(&zero, &oracle, &token, &treasury, &6000u32, &8000u32, &1u32, &0u64);
}
...
fn test_initialize_zero_ltv_fails() {
    let (env, cid, admin, oracle, token, treasury) = setup();
    let client = StellarKraalClient::new(&env, &cid);
    client.initialize(&admin, &oracle, &token, &treasury, &0u32, &8000u32, &1u32, &0u64);
}
...
fn test_initialize_ltv_above_max_fails() {
    let (env, cid, admin, oracle, token, treasury) = setup();
    let client = StellarKraalClient::new(&env, &cid);
    client.initialize(&admin, &oracle, &token, &treasury, &9001u32, &9500u32, &1u32, &0u64);
}
...
fn test_initialize_liq_below_ltv_fails() {
    let (env, cid, admin, oracle, token, treasury) = setup();
    let client = StellarKraalClient::new(&env, &cid);
    client.initialize(&admin, &oracle, &token, &treasury, &6000u32, &5000u32, &1u32, &0u64);
}
...
fn test_initialize_valid_params_succeed() {
    let (env, cid, admin, oracle, token, treasury) = setup();
    let client = StellarKraalClient::new(&env, &cid);
    client.initialize(&admin, &oracle, &token, &treasury, &6000u32, &6000u32, &1u32, &0u64);
}
...
// ── Issue #705: Configurable TWAP window ──────────────────────────────
//
// These tests verify that:
//   1. `initialize` accepts a custom `twap_window_ledgers` value.
//   2. Passing `0` applies the 720-ledger default.
//   3. `set_twap_window` lets the admin update the window after init.
//   4. `set_twap_window(0)` returns `InvalidAmount`.
//   5. The TWAP calculation respects the configured window — a price
//      submitted after the window expires starts a fresh accumulation.

/// Verify that `initialize` stores the provided custom TWAP window.
#[test]
fn test_initialize_custom_twap_window() {
    let (env, cid, admin, oracle, token, treasury) = setup();
    let client = StellarKraalClient::new(&env, &cid);
    client.initialize(&admin, &oracle, &token, &treasury, &6000u32, &8000u32, &1u32, &1440u64);
    assert_eq!(
        client.get_twap_window(),
        1440,
        "TWAP window must equal the value passed to initialize"
    );
}

/// When `twap_window_ledgers = 0` is passed, the default of 720 is applied.
#[test]
fn test_initialize_default_twap_window_when_zero() {
    let (env, cid, admin, oracle, token, treasury) = setup();
    init(&env, &cid, &admin, &oracle, &token, &treasury);
    let client = StellarKraalClient::new(&env, &cid);
    assert_eq!(
        client.get_twap_window(),
        720,
        "TWAP window must default to 720 when 0 is passed to initialize"
    );
}

/// Admin can update the TWAP window after initialization via `set_twap_window`.
#[test]
fn test_set_twap_window_by_admin() {
    let (env, cid, admin, oracle, token, treasury) = setup();
    init(&env, &cid, &admin, &oracle, &token, &treasury);
    let client = StellarKraalClient::new(&env, &cid);

    client.set_twap_window(&admin, &2880u64); // ~4 hours
    assert_eq!(
        client.get_twap_window(),
        2880,
        "TWAP window must be updated to 2880 by the admin"
    );
}

/// `set_twap_window(0)` must return `InvalidAmount` (error code #8).
#[test]
#[should_panic(expected = "#8")]
fn test_set_twap_window_zero_fails() {
    let (env, cid, admin, oracle, token, treasury) = setup();
    init(&env, &cid, &admin, &oracle, &token, &treasury);
    let client = StellarKraalClient::new(&env, &cid);
    client.set_twap_window(&admin, &0u64);
}

/// Non-admin cannot update the TWAP window (error code #3 = Unauthorized).
#[test]
#[should_panic(expected = "#3")]
fn test_set_twap_window_unauthorized() {
    let (env, cid, admin, oracle, token, treasury) = setup();
    init(&env, &cid, &admin, &oracle, &token, &treasury);
    let client = StellarKraalClient::new(&env, &cid);
    let attacker = Address::generate(&env);
    client.set_twap_window(&attacker, &1000u64);
}

/// Verify that the TWAP accumulation resets when a price is submitted
/// after the configured window has elapsed.
///
/// Scenario:
///   - Window = 100 ledgers
///   - Submit price 100 at t=0  → TWAP = 100
///   - Submit price 200 at t=50 → TWAP = (100 + 200) / 2 = 150
///   - Advance time by 101 ledgers (window expires)
///   - Submit price 300 at t=151 → TWAP resets to 300
#[test]
fn test_twap_calculation_respects_window() {
    let (env, cid, admin, oracle, token, treasury) = setup();
    // Use a small window (100 ledgers) for easy testing
    let client = StellarKraalClient::new(&env, &cid);
    client.initialize(&admin, &oracle, &token, &treasury, &6000u32, &8000u32, &1u32, &100u64);

    // t = 0: submit 100
    client.submit_price(&oracle, &100i128);
    let d1 = client.get_twap_data();
    assert_eq!(d1.twap_price, 100, "TWAP after first price should be 100");
    assert_eq!(d1.current_price, 100);

    // t = 50: submit 200 (still within window)
    env.ledger().with_mut(|li| { li.timestamp += 50; });
    client.submit_price(&oracle, &200i128);
    let d2 = client.get_twap_data();
    assert_eq!(d2.twap_price, 150, "TWAP after two prices should be (100+200)/2 = 150");

    // t = 151: submit 300 (window has expired → reset)
    env.ledger().with_mut(|li| { li.timestamp += 101; });
    client.submit_price(&oracle, &300i128);
    let d3 = client.get_twap_data();
    assert_eq!(
        d3.twap_price, 300,
        "TWAP must reset to new price when window expires"
    );
    assert_eq!(d3.current_price, 300);
}

/// Verify that changing the window via `set_twap_window` affects subsequent
/// TWAP calculations.
#[test]
fn test_twap_changes_with_different_window() {
    let (env, cid, admin, oracle, token, treasury) = setup();
    init(&env, &cid, &admin, &oracle, &token, &treasury);
    let client = StellarKraalClient::new(&env, &cid);

    // Submit one price to initialise TWAP
    client.submit_price(&oracle, &100i128);

    // Narrow the window to 10 ledgers so the next price (at t=11) resets it
    client.set_twap_window(&admin, &10u64);
    assert_eq!(client.get_twap_window(), 10);

    env.ledger().with_mut(|li| { li.timestamp += 11; });
    client.submit_price(&oracle, &500i128);

    let data = client.get_twap_data();
    // Window expired → TWAP resets to the latest price
    assert_eq!(
        data.twap_price, 500,
        "After window change and expiry, TWAP must restart at new price"
    );
}
