...
impl StellarKraal {
    ...
    /// # TWAP Window
    ///
    /// `twap_window_ledgers` sets the number of ledgers (approximately
    /// seconds on Stellar) over which the time-weighted average price is
    /// calculated.  Pass `0` to use the default of **720 ledgers**
    /// (~1 hour at ~5 s/ledger).  The admin can update this value later
    /// via [`StellarKraal::set_twap_window`].
    ///
    /// # Migration note
    ///
    /// This parameter was added in v1.1.0.  Existing deployments that call
    /// `initialize` without this argument must be redeployed; the contract
    /// ABI changes are non-breaking when `twap_window_ledgers = 0` (default
    ...
    pub fn initialize(
        ...
        env.storage().instance().set(&TWAP_WINDOW, &twap_window);
